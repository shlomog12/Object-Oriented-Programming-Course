...
class WGraph_AlgoTest {
    ...
    void runTime() {
        ...
        weighted_graph g10 = buildMillion();
        long end10Million = new Date().getTime();
        double lengthTime10Million = (end10Million - start10Million) / 1000.0;
        System.out.println("time for graph Million nodes and 10Million edge  = " + lengthTime10Million);
        assertTrue(lengthTime10Million <= 20, "a long time for 10milion: " + lengthTime10Million + ">20");
        g10 = null;
        long startMillion = new Date().getTime();
        int nodes = 100000;
        int esges = nodes * 10;
        weighted_graph g = build(nodes, esges);
        WGraph_Algo al = new WGraph_Algo();
        al.init(g);
        WGraph_DS gGet = (WGraph_DS) al.getGraph();
        WGraph_DS gCopy = (WGraph_DS) al.copy();
        boolean connected = al.isConnected();
        double path1 = al.shortestPathDist(3, 10000);
        double path2 = al.shortestPathDist(24, 864);
        List<node_info> l1 = al.shortestPath(15, 2000);
        ...
        System.out.println("Time for algorithms on a graph of one hundred thousand nodes and one million edges  = " + lengthTimeMillion);
        assertTrue(lengthTimeMillion < 20, "a long time for milion: " + lengthTimeMillion + ">20");
    }


    private static WGraph_DS buildMillion() {
        int nodeSize = (int) Math.pow(10, 6);
        int edgeSize = (int) Math.pow(10, 7);
        WGraph_DS g = new WGraph_DS();
    ...
    private static WGraph_DS build(int nodeSize, int edgeSize) {
        ...
        Random rndNeighbor2 = new Random(2);
        Random rndWight = new Random(1);
